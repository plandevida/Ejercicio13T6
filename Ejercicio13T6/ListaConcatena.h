/**
 ...
 Created by Dani on 13/03/13.
 */

#ifndef Ejercicio10t6_ListaConcatena_h
#define Ejercicio10t6_ListaConcatena_h
...
public:
	...
	ListaConcatena() : _prim(NULL), _ult(NULL), _numElems(0) {}
	...
	/** Constructor copia */
	ListaConcatena(const ListaConcatena<T> &other) : _prim(NULL), _ult(NULL) {
		copia(other);
	}
    
	/** Operador de asignaciÛn */
	ListaConcatena<T> &operator=(const ListaConcatena<T> &other) {
		if (this != &other) {
			libera();
			copia(other);
		}
	...
	bool operator==(const ListaConcatena<T> &rhs) const {
...
protected:
	...
	void copia(const ListaConcatena<T> &other) {
